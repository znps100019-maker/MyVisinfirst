...
def handle_non_ascii_path():
    ...
    try:
        result = subprocess.run(
            [virtual_python, virtual_script] + sys.argv[1:], env=child_env
        )
...
def build_args():
    parser = argparse.ArgumentParser(description="Recognize trained sign-language labels.")
    parser.add_argument(
        "--input",
        default="0",
        help="Input source: camera index (e.g. 0), local video file path, or YouTube URL.",
    )
    ...
    parser.add_argument("--no-srt", action="store_true", help="Do not generate .srt subtitle file (video only).")
    parser.add_argument("--no-txt", action="store_true", help="Do not generate .txt timeline file (video only).")
    parser.add_argument("--min-duration", type=float, default=0.2, help="Minimum duration of a gesture segment (video only).")
    parser.add_argument("--headless", action="store_true", help="Run without opening a GUI window.")
    parser.add_argument("--max-frames", type=int, default=0, help="Stop after this many frames (video only).")
    parser.add_argument("--frame-interval", type=int, default=1, help="Process one frame every N frames to speed up (video only).")
...
def load_model(model_name):
    ...
    if not os.path.exists(model_path):
        ...
        print("Please train a model first (downloader.py -> extract_dataset.py -> train_classifier.py)")
        sys.exit(1)
...
def main():
    ...
    is_camera = False
    try:
        camera_idx = int(args.input)
        is_camera = True
    except ValueError:
        camera_idx = None

    if is_camera:
        print(f"Opening camera index {camera_idx}...")
        # Use DirectShow on Windows to open faster
    ...
    if not cap.isOpened():
        print(f"Cannot open input source: {args.input}")
        sys.exit(1)
    ...
    try:
        while True:
            ...
            if not is_camera and frame_idx % args.frame_interval != 0:
                frame_idx += 1
                continue
            frame_idx += 1

            # Camera inputs are flipped by default for mirror view
            if is_camera:
            ...
            if not is_camera and args.max_frames > 0 and frame_idx >= args.max_frames:
                print(f"Reached max frames: {args.max_frames}")
                break
    finally:
        # Wrap up the final segment
        if not is_camera and active_segment:
            start_t = active_segment["start_time"]
            end_t = frame_idx / fps
            if end_t - start_t >= args.min_duration:
                segments.append({
        ...
        # Output timeline results if running on a video file
        if not is_camera and segments:
            script_dir = os.path.dirname(os.path.abspath(__file__))
