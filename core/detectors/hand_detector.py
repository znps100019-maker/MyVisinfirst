...
class HandSignRecognizer:
    ...
    def __init__(self, max_num_hands=2, history_size=8, stable_min_count=3):
        ...
        self.knn_samples = []
        try:
            model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "sign_language_app", "model.json"))
            if os.path.exists(model_path):
                with open(model_path, "r", encoding="utf-8") as f:
                    self.knn_samples = json.load(f).get("samples", [])
        except Exception as e:
            print(f"Warning: Could not load KNN model.json: {e}")
    ...
    def process(self, frame):
        ...
        # 全局 KNN 辨識 (雙手聯合特徵 126維)
        global_knn_sign = "Unknown"
        if self.knn_samples:
            vector = self._normalize_landmarks(results.multi_hand_landmarks)
            global_knn_sign = self._classify_knn(vector, distance_threshold=1.5)
        ...
        # 優先採用全局 KNN (雙手 126D) 的結果
        if global_knn_sign != "Unknown":
            self.history.append(global_knn_sign)
        else:
            if len(detections) >= 2:
                # 數學規則回退
    ...
    def _normalize_landmarks(self, multi_hand_landmarks):
        ...
        scale = math.sqrt(dx**2 + dy**2 + dz**2)
        if scale == 0:
            scale = 1e-6
    ...
    def _classify_knn(self, vector, k=9, distance_threshold=1.5):
        ...
        k = min(k, len(self.knn_samples))
        neighbors = []
        for sample in self.knn_samples:
            sv = sample["vector"]
            dist = math.sqrt(sum((a - b) ** 2 for a, b in zip(vector, sv)))
            neighbors.append((dist, sample["label"]))
            
        neighbors.sort(key=lambda x: x[0])
        top_k = neighbors[:k]
        
        # 嚴格模式：如果最接近的特徵點距離仍然大於門檻，代表長得不像任何資料庫裡的手勢
        if top_k[0][0] > distance_threshold:
            return "Unknown"
